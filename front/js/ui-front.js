...
ui.Confetti = {
  set: function (wrap) {
    ...
    if ($wrap.data('length')) $itemLength = $wrap.data('length');
...
const canvasLightning = function (c, cw, ch) {
  ...
  this.renderL = function () {
    ...
    while (i--) {
      const light = this.lightning[i];
      this.ctx.strokeStyle = 'rgba(255, 255, 255, ' + this.rand(10, 100) / 100 + ')';
  ...
  $(window).on('resize', function () {
    _this.cw = _this.c.width = _this.c.offsetWidth;
    _this.ch = _this.c.height = _this.c.offsetHeight;
  });
