...
- 실서버에는 front.min.css 파일만 배포하며<br />scss 파일 및 front.css.map 파일은 실서버에 배포하지 않아도 됩니다.
- 일부 단색의 아이콘의 경우 이미지를 직접사용하지 않고<br />소스코드 (위 'svg 이미지를 소스코드(bg)로 전환' 참고) 로 변환하여<br />scss파일에서 직접사용합니다.
- 단위는 px를 사용하지않고 rem으로 작성합니다.
- layout에 속하는 #wrap(.page로 대체가능),<br />#header(.page-head로 대체가능), #container(.page-body로 대체가능) 빼고는<br />대부분 클래스(class)기반으로 스타일이 적용되어있습니다.
...
### 기타   
- <u>pub폴더의 파일들은 코딩리스트(퍼블진행현황리스트) 관련 파일이므로 해당 프로젝트와 크게 상관없습니다.</u><br />배포할 필요도 없습니다.
...
- images 폴더안에 guide폴더(guide용) 및 temp폴더(임시용)의 이미지는 <br />디자인팀에 제공받은 이미지가 아니거나 <br />실사용하는 이미지가 아니므로 <br /><u>최대한 사용하지않는게</u> 좋습니다.
- 한글은 noto sans, 숫자는 roboto로 <br />_font.scss에서 unicode-range를 이용하여 구분하여 자동적용시켰습니다. <br />','는 (큰)숫자사이에 들어가는 케이스때문에 roboto로 적용되었습니다.
- noto sans는 웹폰트로써 (용량문제로) 최적화 되어있기때문에 모든 한글케이스에 적용되지 않습니다.
- 폰트는 디자인상 대부분 Regular, Medium, Bold 폰트만 이용하였고,<br />일부 매치(결과)화면에서 지마켓 폰트가 사용되었습니다.
