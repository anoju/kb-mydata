...
## html 마크업(퍼블리싱) 참고
<u>아래 내용은 개발이 아닌 퍼블리싱 참고 내용입니다.</u>
...
### 파일
- static 폴더안에 파일만 작성 및 수정합니다.<br />경로: /src/main/webapp/resources/static
- 폴더 구조는 개발팀과 협의하여 작업되었습니다.
- static > images 폴더안에 guide폴더(guide용) 및 temp폴더(임시용)의 이미지는 <br />디자인팀에 제공받은 이미지가 아니거나 실사용하는 이미지가 아니므로 <br /><u>최대한 사용하지않는게</u> 좋습니다.
- static > images 폴더안에 이미지 중 이름에 '@3x' 들어간 이미지도 디자인팀에 제공받은 이미지가 아닙니다. 실사용 시 재디자인이 필요합니다.
- static > pub폴더의 파일들은 코딩리스트(퍼블진행현황리스트) 관련 파일이므로 <u>해당 프로젝트와 크게 상관없습니다.<br />배포할 필요도 없습니다.</u>
- static > temp폴더 역시 임시 파일이므로 <u>최대한 사용하지않는게</u> 좋습니다.
- 배포시 실서버에는 front.min.css 파일만 배포하며<br />scss 파일 및 front.min.css.map 파일은 실서버에 배포하지 않아도 됩니다.(배포해도 크게 상관은 없음)
<br />
...
- 'ui-'로 시작하는 클래스는 기능용(스크립트) 클래스 입니다.<br />불필요한 기능일때는 꼭 지우고 사용해야 합니다.
<br />
...
- 인터렉션은 로띠(lottie)를 제외 한 나머지 대부분은 <br />퍼블파트에서 고려해서 만들었습니다.
<br />

### 산출물 
> <https://anoju.github.io/kyobo-mydata-pub/pub/index.html>
- front화면 디자인(피그마)을 최대한 참고하여 작업되었니다.
- front는 별도의 퍼블가이드 html이 있습니다.<br />위링크에서 확인가능합니다.
- front 퍼블가이드 중에는 제 경험기반 최대한으로 기능 및 확정성 위주로 만들어져있기때문에<br />디자인상 없는 디자인과 사용되지 않은 기능도 존재합니다. (예: 달력기능 등등)<br />추후 퍼블가이드상 <u>디자인산출물에 없는 디자인을 쓰게 될 경우</u>(아이콘의 경우 등) <br />새롭게 재디자인을 해서 사용해야합니다.
<br />
